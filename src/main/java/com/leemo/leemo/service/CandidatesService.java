...
public class CandidatesService {
    ...
    public void respond(Long taskId, String email) throws Exception {
        List<Candidates> allExTasks = candidatesRepository.findCandidatesByExecutor(email);    Candidates candidate = new Candidates();    for (Candidates c : allExTasks) {
            if (c.getTaskId().equals(taskId)) {
                throw new Exception("Error");        } else {
                candidate.setExecutor(email);            candidate.setTaskId(taskId);            candidatesRepository.save(candidate);        }
        }
    }
